...
Codex plugin and MCP connector for XMem persistent memory.

This package connects Codex to XMem through the XMem MCP server. It writes local connector configuration and agent-facing memory instructions, while keeping XMem credentials in environment variables.

## Install

```bash
npx xmem-codex@latest install
```

For local development:

```bash
node src/cli.js install --config-root ./tmp/codex
```

## Authentication

Set credentials in your shell, OS secret store, or the client environment before launching Codex:
...
| Command | Description |
| --- | --- |
| `install` | Write .codex/config.toml, AGENTS.md, and Codex plugin files. |
